...
async def test_setup_entry(hass: HomeAssistant, mock_config_entry: ConfigEntry):
    ...
    with patch(
        "custom_components.xiaozhi_mcp.coordinator.XiaozhiMCPCoordinator"
    ) as mock_coordinator_class, patch(
        "custom_components.xiaozhi_mcp.mcp_client.XiaozhiMCPClient"
    ) as mock_client_class, patch(
        "homeassistant.config_entries.ConfigEntries.async_forward_entry_setups"
    ) as mock_forward_setups, patch(
        "homeassistant.helpers.device_registry.async_get"
    ) as mock_device_registry, patch(
        "custom_components.xiaozhi_mcp._async_setup_services"
...
async def test_unload_entry(hass: HomeAssistant, mock_config_entry: ConfigEntry):
    ...
    ) as mock_unload_platforms, patch(
        "homeassistant.helpers.device_registry.async_get"
    ) as mock_device_registry, patch(
        "custom_components.xiaozhi_mcp._async_setup_services"
    ) as mock_setup_services:
